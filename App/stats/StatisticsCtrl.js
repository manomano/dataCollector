(function () {
  let statisticsController = function (
    ...
    let till =
      "2020" + "-" + addZero(today.getMonth()) + "-" + addZero(today.getDay());
    ...
    $scope.getData = function () {
      $scope.params.from =
        $scope.fromField.getFullYear() +
        "-" +
        addZero($scope.fromField.getMonth()) +
        "-" +
        addZero($scope.fromField.getDate());
      $scope.params.till =
        $scope.tillField.getFullYear() +
        "-" +
        addZero($scope.tillField.getMonth()) +
        "-" +
        addZero($scope.tillField.getDate());

      console.log($scope.params);
      simpleObjectService.getStatistics($scope.params).then(
        function (response) {
