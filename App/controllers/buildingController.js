(function () {
  var buildingControllerService = function (
    $rootScope,
    ...
    function selectFilter(feature, layer) {
      return (
        (layer.get("name") == "buildings" &&
          feature.get("PARCEL_ID") == me.parcel_id) ||
        (!me.parcel_id && layer.get("name") == "existingBuildings")
      );
    }
    ...
    function setIsactive(act) {
      ...
      if (act) {
        ...
        me.featureSelector = featureSelectorFactory(
          $rootScope.map.map,
          $rootScope.map.layers,
          $rootScope.layerOrg.selectFilter({
            layers: [
              "buildings",
              "existingObjects",
              "all",
    ...
    this.next = function () {
      simpleObjectService
        .getObjectCount("buildings", {
          x: me.parcel_id,
          PARCEL_ID: me.parcel_id,
        })
        .then(
          function (response) {
            if (response.data < me.allowed_count) {
              MessagingService.displayError(
                "შენობების რაოდენობა ნაკლებია მითითებულზე"
              );
              return;
            }

            if (!me.buildingEntrances_count) {
              simpleObjectService.getFeatures("buildings", me.parcel_id).then(
                function (response) {
                  var buildingfeatures = [];
                  response.data.features.map(function (x) {
                    let geoJ = new ol.format.GeoJSON({
    ...
    this.updateToastTitle = function (additionalInfo) {
      $rootScope.listenerObject.currentTitle =
        "შენობების ხაზვა  " + additionalInfo;
    };
    ...
    this.back = function (geometry, rawFeature) {
      ...
      FormService.allowDataEntrance(
        me.title,
        { stepName: me.infoName, id: me.building_id },
        null,
        formCallBack,
        me.existingFeatureProperties
      );
    };
    ...
    this.getObjectCount = function () {
      return $rootScope.layerOrg.getObjectCount("buildings", {
        x: me.parcel_id,
        PARCEL_ID: me.parcel_id,
      });
    };
    ...
    this.draw = function () {
      if (me.allowed_count <= me.getObjectCount()) {
        MessagingService.displayError(
          "შენობების ოდენობა ემთხვევა მითითებულს, მეტის დამატების უფლება არ გაქვთ"
        );
        return;
      }
      setIsactive(false);
      ...
      if (!this.featureToSnap) {
        this.featureToSnap = $rootScope.layerOrg.search(
          "parcels",
          me.parcel_id
        );
      }
      DrawingControllerService.init(
        this.name,
        this.featureToSnap,
        me.parcel_id
      );
      DrawingControllerService.turnOn();
      DrawingControllerService.subscriber.addCallback(
        DrawingControllerService,
        f
      );
    };
    ...
    this.subStep = function () {
      ...
      setIsactive(false);
      buildingEntranceControllerService.init({
        building_id: me.building_id,
        map: me.map,
        allowed_count: me.buildingEntrances_count,
        isLast: isLast,
        featureToSnap: me.rawFeature,
      });
      buildingEntranceControllerService.turnOn();
      if (!isLast) {
        buildingEntranceControllerService.subscriber.addCallback(
          buildingEntranceControllerService,
          backFromChild
        );
      } else {
    ...
    this.copyGeometry = function () {
      ...
      me.existingFeatureProperties = $rootScope.layerOrg.getFeatureProperties(
        "existingBuildings",
        feature.getProperties()["ID"],
        feature
      );

      var assignmentFeature =
        $rootScope.layerOrg.getAssignmentFeature("assignments");
      assignmentFeature = assignmentFeature.length
        ? assignmentFeature[0]
        : assignmentFeature;
      if (assignmentFeature) {
        if (
          parseInt(me.existingFeatureProperties.BLOCK_ID) !==
          parseInt(assignmentFeature.getProperties().BLOCK_ID)
        ) {
          MessagingService.displayError(
            "მონიშნული შენობა ეკუთვნის სხვა დავალებას"
          );
          return;
        }
      }
      ...
      setIsactive(false);
      DrawingControllerService.initEdit(
        this.name,
        feature,
        null,
        false,
    ...
    this.edit_general = function (event) {
      ...
      if (
        !feature ||
        !(
          feature.getProperties().name == "buildings" ||
          feature.getProperties().featureType == "building"
        )
      ) {
        MessagingService.displayError(
          "საჭიროა შენობის მონიშვნა შენობის ფენიდან"
        );
        return;
      }
    ...
    this.deleteObject = function (ev) {
      ...
      var id = me.featureSelector.getSelectedFeatures()[0].getProperties()[
        "ID"
      ];
      var actualCount = me.getObjectCount();
    ...
    this.editEntrances = function () {
      ...
      var callback = function (data) {
        me.buildingEntrances_count = data.geometry
          ? feature.get("BUILDING_ENTRANCES_NUM")
          : data.BUILDING_ENTRANCES_NUM;

        if (me.buildingEntrances_count == 0) {
          MessagingService.displayError(
            "საჭიროა ამ შენობის ფორმის ველების შევსება"
          );
          return;
        }
      ...
      var data = $rootScope.layerOrg.getFeatureProperties(
        "buildings",
        me.building_id,
        callback
      );

      me.buildingEntrances_count = data.N
        ? feature.get("BUILDING_ENTRANCES_NUM")
        : data.BUILDING_ENTRANCES_NUM;
    ...
    this.split = function () {
      ...
      features.map((x) =>
        FeatureArr.push({
          name: x.get("featureType") || x.get("name"),
          id: x.get("ID"),
        })
      );
  ...
  buildingControllerService.$inject = [
    "$rootScope",
    "buildingEntranceControllerService",
    "DrawingControllerService",
    "$mdDialog",
  ...
  angular
    ...
    .service("buildingControllerService", buildingControllerService);
})();
