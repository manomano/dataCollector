...
(function () {
    var DrawingControllerService = function ($rootScope,MessagingService, ParcelService, simpleObjectService) {
        var isactive = false;
        var me = this;
        ...
        function setIsactive(act){
            ...
            if(!act){
                ...
                $rootScope.map.map.removeInteraction(me.featureSelector.modifyInteraction);
                $rootScope.map.map.removeInteraction(me.featureSelector.dragInteraction);
        ...
        this.init = function (interactionName, featureToSnap, id) {
            ...
            this.editing = false;
            this.id = id;
            $rootScope.map.createDrawingLayer(interactionName=='trafficSignGeometry'?Styles.trafficSignFeatureStyle:null);
            $rootScope.map.featureDrawer.onDrawnAction.addCallback($rootScope.map.featureDrawer,DrawEndCallBack)
            this.featureSelector = new WFSFeatureSelector($rootScope.map.map, $rootScope.map.layers);
        ...
        this.initEdit = function (interactionName, feature, featureToSnap, editing, id) {
            this.saveButtonState = false;
        ...
        this.saveGeometry = function () {
            ...
            var feature = JSON.parse(utils.convertFeature($rootScope.map.DrawingLayer.getSource().getFeatures()[0]));
            if(me.id){
                feature.id = me.id;
            }
            ...
            if(['parcelEntranceGeometry', 'parcelGeometry', 'buildingGeometry', 'buildingEntranceGeometry'].indexOf(me.interactionName )>=0){
                ParcelService.router(me.editing, me.interactionName , feature).then(function (response) {
                    setIsactive(false);
                ...
                var id = this.id?this.id:(feature.properties?feature.properties.ID:null);

                simpleObjectService.router({
        ...
        this.stopDrawing = function () {
            if(!$rootScope.map.featureDrawer.drawingInteractions[utils.featureNameTypeMap[me.interactionName]].a){
                return;
            }
            if($rootScope.map.featureDrawer.drawingInteractions[utils.featureNameTypeMap[me.interactionName]].a[0].length<4){
                return;
            }
            $rootScope.map.featureDrawer.drawingInteractions[utils.featureNameTypeMap[me.interactionName]].finishDrawing();
        }
    }


    DrawingControllerService.$inject = ["$rootScope","MessagingService","ParcelService","simpleObjectService"];
    angular.module("datacollection.services").service("DrawingControllerService", DrawingControllerService);
