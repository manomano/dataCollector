(function () {
    ...
    var formController = function ($sce, $scope, $timeout, MessagingService, StorageService, $mdDialog, FormService, ParcelService, conf, metadata, existingParcelProperties) {
        var self = this;
        $scope.generalForm = {};
        $scope.conf = conf;
        ...
        $scope.category_change = function (selectedValue, index, field) {
            ...
            if ($scope.fields[index + 1] && $scope.fields[index + 1].field_id == $scope.fields[index].field_id) {
            ...
            while ($scope.fields[nextIndex] && $scope.fields[index].field_id == $scope.fields[nextIndex].field_id) {
        ...
        $scope.traverseItems = function (items, parentObject) {
            ...
            if ($scope.metadata) {
                if ($scope.metadata[parentObject.id]) {
                    ...
                    var rec;
                    while (rec = items.find(function (x) {
            ...
            for (i = 0; i < maxLevel; i++) {
                ...
                labelArr[1] = (i > 0 ? 'ქვე' : '') + labelArr[1] + ' ' + (i > 0 ? i : '');
        ...
        $scope.addModelField = function (field, parentId) {
            ...
            if ($scope.existingParcelPropertiesObject) {
                field.model = $scope.existingParcelPropertiesObject[field.id] ? (field.type == 'NUMBER_FIELD' ? parseFloat($scope.existingParcelPropertiesObject[field.id]) : $scope.existingParcelPropertiesObject[field.id]) : "";

                if(field.type == 'REPEATABLE_TEXT_FIELD') {
                    if($scope.existingParcelPropertiesObject[field.id]){
                        field.model = $scope.existingParcelPropertiesObject[field.id].split("|").map(x=>x = {OWNER:x})
                    }
                }
            }
            ...
            if ($scope.metadata) {
                ...
                if(field.type == 'REPEATABLE_TEXT_FIELD'){
                    ...
                    if($scope.metadata[field.id]=='დაუდგენელი' || !$scope.metadata[field.id]){
                        field.unidentified = true;
                        field.checked = true;
                        field.model = [];
                    }else {
                        field.model = $scope.metadata[field.id].split("|").map(x=>x = {OWNER:x})
                    }
        ...
        $scope.showHide = function (field) {
            ...
            return !field.hasOwnProperty('parentId') || $scope.treeIdies.indexOf(field.parentId) >= 0
        };
        ...
        $scope.save = function (valid) {
            ...
            if ($scope.fields.filter(x => (x.required == 3 && !x.model) || (x.required == 2 && !x.model && x.type !== 'NUMBER_FIELD') || (x.required == 2 && !(parseInt(x.model) + 1) && x.type == 'NUMBER_FIELD')).length > 0 || !valid) {
            ...
            for (var i in $scope.fields) {
                ...
                if ($scope.fields[i].type == 'CATEGORY_SELECT_FIELD') {
                    ...
                    if($scope.fields[i].type == 'REPEATABLE_TEXT_FIELD'){
                        ...
                        $scope.parameters[$scope.fields[i].id] = $scope.fields[i].unidentified?'დაუდგენელი':$scope.fields[i].model.map(x=>x=x['OWNER']).join("|");
            ...
            if (['busStops', 'trafficSigns'].indexOf($scope.conf.stepName) < 0) {
                ParcelService.router($scope.parameters.editing, $scope.conf.stepName, $scope.parameters).then(function () {
        ...
        if (StorageService.getValue($scope.conf.stepName)) {
            ...
            StorageService.setvalue($scope.conf.stepName, JSON.stringify($scope.tree));
        } else {
    ...
    formController.$inject = ["$sce", "$scope","$timeout", "MessagingService", "StorageService", "$mdDialog", "FormService", "ParcelService","conf", "metadata", "existingParcelProperties"];
    angular.module('datacollection.mypanel').controller("FormCtrl", formController);
