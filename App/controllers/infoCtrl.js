(function () {
  var infoController = function (
    ...
    if (["pois", "busStops", "trafficSigns"].indexOf(stepName) >= 0) {
      simpleObjectService.getPhotoIdies(additional_info.id).then(
        function (response) {
          for (var i in response.data) {
            $scope.photoURLArr.push({
    ...
    for (var i = 0; i < $scope.metadata.length; i++) {
      $scope.fields.push({
        label: $scope.metadata[i]["key"],
        value: $scope.metadata[i]["value"],
      });
    }
    ...
    $scope.getErrors = function () {
      ...
      for (prop in $scope.metadata) {
        let val =
          typeof $scope.metadata[prop] === "boolean"
            ? yesNo[$scope.metadata[prop] + 0]
            : $scope.metadata[prop];
        $scope.fields.push({
          label: dict[prop],
          value:
            typeof $scope.metadata[prop] == "object"
              ? $scope.metadata[prop].join(",")
              : val,
        });
      }
    };
