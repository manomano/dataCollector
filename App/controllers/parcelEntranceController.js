(function () {
  var parcelEntranceControllerService = function (
    $rootScope,
    ...
    function selectFilter(feature, layer) {
      return (
        layer.get("name") == "parcelEntrances" &&
      ...
      if (act) {
        ...
        me.featureSelector = featureSelectorFactory(
          ...
          $rootScope.layerOrg.selectFilter({
            layers: ["parcelEntrances", "all"],
            layerName: "parcelEntrances",
            key: "PARCEL_ID",
            value: me.parcel_id,
          })
        );
    ...
    this.updateToastTitle = function (additionalInfo) {
      $rootScope.listenerObject.currentTitle =
        "ნაკვეთის შესასვლელების ხაზვა  " + additionalInfo;
    };
    ...
    this.next = function () {
      simpleObjectService
        .getObjectCount("parcelEntrances", {
          x: me.parcel_id,
        ...
        .then(
          function (response) {
            ...
            simpleObjectService
              .metadataIsFilled("parcels", me.parcel_id)
    ...
    this.getObjectCount = function () {
      return $rootScope.layerOrg.getObjectCount("parcelEntrances", {
        x: me.parcel_id,
        PARCEL_ID: me.parcel_id,
      });
    };
    this.draw = function () {
      if (me.allowed_count <= me.getObjectCount()) {
        MessagingService.displayError(
          "ნაკვეთის შესასვლელების ოდენობა ემთხვევა მითითებულს, მეტის დამატების უფლება არ გაქვთ"
        );
        return;
      }
  ...
  parcelEntranceControllerService.$inject = [
    "$rootScope",
    "DrawingControllerService",
    "$mdDialog",
    "ParcelService",
