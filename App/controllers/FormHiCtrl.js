(function () {
    ...
    var formController = function ($sce, $scope, MessagingService, StorageService,  $mdDialog, FormService, stepName, metadata,existingParcelProperties) {
        var self = this;
        $scope.treeIdies = [];
        $scope.fields = [];
        ...
        $scope.category_change = function (selectedValue, index, field) {
            ...
            $scope.fields = $scope.fields.filter(x=>$scope.treeIdies.indexOf(x.parentId)>=0 || x.parentId==-1 || !x.parentId);
            ...
            function fill(br) {
                ...
                var o = {label: branch.geoName + ' - ქვეკატეგორია',geoName:branch.geoName, id:branch.id, parentId:branch.id, items:branch.children, field_id:branch.field_id, type: "CATEGORY_SELECT_FIELD"};
            ...
            while($scope.treeIdies[k]){
                branch = JSON.parse(JSON.stringify(branch.children.find(x=>x.id==$scope.treeIdies[k])));
                if(!branch.children){
                    break;
                }
                branch.children.map(function (x) {
            ...
            function replaceTextAreaAtLastIndex() {
                if($scope.fields[$scope.fields.length-1].type!=='TEXTAREA_FIELD'){
                    var textareaIndex = $scope.fields.findIndex(x=>x.type=='TEXTAREA_FIELD');
                    if(textareaIndex>=0){
        ...
        $scope.addModelField = function (field, parentId) {
            ...
            if($scope.metadata){
                field.model = field.type=='NUMBER_FIELD'? parseFloat($scope.metadata[field.id]):$scope.metadata[field.id];
                if(field.type=='TIME_FIELDS'){
                    field.times.from = "";
                    field.times.to = "";
                    field.model = null;
                    if($scope.metadata[field.id]){
                        ...
                        if($scope.metadata[field.id].indexOf("-")>=0){
                            field.times.from = new Date(new Date().getUTCFullYear(), new Date().getMonth(), new Date().getDay(), parseInt(timeArr[0].split(":")[0]), parseInt(timeArr[0].split(":")[1]))  ;
                            if(timeArr[1]){
                                field.times.to = new Date(new Date().getUTCFullYear(), new Date().getMonth(), new Date().getDay(), parseInt(timeArr[1].split(":")[0]), parseInt(timeArr[1].split(":")[1]));
                            }
                        }else{
                ...
                if(field.type=='PHONE_FIELD'){
                    if($scope.metadata[field.id]){
                        field.model = $scope.metadata[field.id].split("|").map(x=>x = {phone:x})
                    }
                }
       ...
       $scope.recursive = function(branch, ind){
           ...
           if(branch){
               if(branch.children.length){
                   ...
                   next = branch.children.find(x=>$scope.metadata.PARENTS.indexOf(x.id)>=0 || x.id==value);
        ...
        $scope.traverseTree = function (tree) {
                ...
                for(var i in branch){
                    if(!branch[i].fields){
                        if(branch[i].type!=='CATEGORY_SELECT_FIELD'){
                            ...
                            parentSelectBoxObject.items.map(function(x){delete x.children; delete x.fields });
                            if($scope.metadata && $scope.metadata.PARENTS){
                                parentSelectBoxObject.model = $scope.metadata.PARENTS[$scope.metadata.PARENTS.length-1]
                            }
                ...
                if($scope.metadata && $scope.metadata.PARENTS){
                    var br = $scope.tree[0].children[0].items.find(x=>x.id==$scope.metadata.PARENTS[$scope.metadata.PARENTS.length-1])
                    $scope.recursive(br, 0);
                    $scope.treeIdies.push(...Object.assign([],$scope.metadata.PARENTS).reverse());
                }
        ...
        $scope.showHide = function (field) {
            return !field.hasOwnProperty('parentId') || $scope.treeIdies.indexOf(parseInt(field.parentId))>=0
        };
        ...
        $scope.save = function (valid) {
            ...
            if (!$scope.generalForm.$valid || $scope.fields.filter(x=>x.field_id ==14 || x.id==14).filter(x=>x.model==0).length>0) {
            ...
            for(var i in $scope.fields){
                ...
                if($scope.fields[i].type=='CATEGORY_SELECT_FIELD'){
                    if(typeof ($scope.fields[i].items)=='undefined' || $scope.fields[i].items.length==0 ){
                        continue;
                    }
                    $scope.parameters[$scope.fields[i].field_id] = $scope.fields[i].model;
                }else{

                    if(!$scope.fields[i].model && (!$scope.fields[i].required==2 || $scope.treeIdies.indexOf($scope.fields[i].parentId)<0) ){
                        continue;
                    }
                    ...
                    if(typeof($scope.fields[i].parentId)=='undefined' || $scope.treeIdies.indexOf($scope.fields[i].parentId)>=0){
                        if($scope.fields[i].type=='TIME_FIELDS' && !$scope.fields[i].model){
                            //var dada = $filter('date')($scope.fields[i].times.from , "HH:mm", '+0400');
                            var from = "";
                            var to = "";
                            if($scope.fields[i].times.from){
                                from = $scope.formatPrefixZero($scope.fields[i].times.from.getHours()) + ":" + $scope.formatPrefixZero($scope.fields[i].times.from.getMinutes())
                            }

                            if($scope.fields[i].times.to){
                                to = $scope.formatPrefixZero($scope.fields[i].times.to.getHours()) + ":" + $scope.formatPrefixZero($scope.fields[i].times.to.getMinutes())
                            }
                        ...
                        $scope.parameters[$scope.fields[i].id] = $scope.fields[i].model==null?"":$scope.fields[i].model;

                        if($scope.fields[i].type=='PHONE_FIELD'){
                            $scope.parameters[$scope.fields[i].id] = $scope.fields[i].model.map(x=>x=x['phone']).join("|");
            ...
            StorageService.setvalue('selectBoxCount', JSON.stringify($scope.selectBoxCount));
        ...
        if(StorageService.getValue(stepName)){
            ...
            if(processing_poi && processing_poi.fields.length && (!$scope.metadata || (Object.keys($scope.metadata).length ==1 && $scope.metadata.PARENTS.length==0))){
        ...
        $scope.$watch('fields', function(newVal, oldVal){
            StorageService.saveObject('processing_poi',{fields:$scope.fields, treeIdies:$scope.treeIdies});
    ...
    formController.$inject = ["$sce","$scope", "MessagingService", "StorageService","$mdDialog","FormService","stepName", "metadata", "existingParcelProperties"];
    angular.module('datacollection.mypanel').controller("FormHiCtrl", formController);
