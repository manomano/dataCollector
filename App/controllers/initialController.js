(function () {
  var initialControllerService = function (
    $window,
    $rootScope,
    StorageService,
    MessagingService,
    ...
    function setIsactive(act) {
      ...
      if (act) {
        ...
        me.featureSelector = new featureSelectorCoord(
          $rootScope.map.map,
    ...
    this.createSimpleObjectsGeneral = function (config) {
      ...
      simpleObjectControllerService.subscriber.addCallback(
        simpleObjectControllerService,
        f
      );
    ...
    this.createPOI = function () {
      this.createSimpleObjectsGeneral({
        id: null,
        serviceName: "pois",
        name: "poiGeometry",
        infoName: "pois",
    ...
    this.createTrafficSign = function () {
      this.createSimpleObjectsGeneral({
        id: null,
        serviceName: "trafficSigns",
        name: "trafficSignGeometry",
        infoName: "trafficSigns",
      });
    };
    ...
    this.createBusStop = function () {
      this.createSimpleObjectsGeneral({
        id: null,
        serviceName: "busStops",
        name: "busStopGeometry",
        infoName: "busStops",
      });
    };
      ...
      function qada() {
        ...
        if (["pois", "trafficSigns", "busStops"].indexOf(name) < 0) {
          MessagingService.displayError(
            "საჭიროა POI/trafficSign/busStation/PEBE ობიექტის მონიშვნა"
          );
          return;
        }
        ...
        $mdDialog
          ...
          .then(
            function (option) {
              setIsactive(false);
              simpleObjectControllerService.init({
                id: selected.get("ID"),
                infoName: name,
                editing: true,
                serviceName: name,
                name: name.substring(0, name.length - 1) + "Geometry",
              });
              simpleObjectControllerService.turnOn();
              simpleObjectControllerService.subscriber.addCallback(
                simpleObjectControllerService,
    ...
    this.editParcel = function () {
      ...
      var callback = function (data) {
        if (data.geometry) {
          result_object.parcelEntrances_count = data.get(
            "PARCEL_ENTRANCES_NUM"
          );
      ...
      var data = $rootScope.layerOrg.getFeatureProperties(
        "parcels",
        result_object.parcel_id,
        callback
      );
      if (!data) {
        MessagingService.displayError(
          "დაფიქსირდა შეცდომა, თქვენ არ გაქვთ უფლება დაარედაქტიროთ მოცემული ნაკვეთი"
        );
        return;
      }
    ...
    this.assignTask = function () {
      ...
      var layerName =
        me.featureSelector.getSelectedFeatures()[0].get("name") ||
        me.featureSelector.getSelectedFeatures()[0].get("featureType") + "s";
      ...
      if (layerName == "roadAssignments") {
        ...
        var assignment_ids = me.featureSelector
    ...
    this.completeTask = function () {
      ...
      assignmentService.completeTask(layerName, assignment_id).then(
        function () {
        ...
        function (response) {
          var txt =
            response.status == 403
              ? "დავალების დასრულება შეუძლებელია, რადგან დაუსრულებელ ობიექტებს შეიცავს"
              : "გაურკვეველი პრობლემაა!";
    ...
    this.getInfo = function () {
      ...
      function getMyInfo() {
        ...
        var layerName =
          feature.get("name") || feature.getProperties().featureType + "s";
        layerName =
          layerName[0].charCodeAt() <= 90
        ...
        simpleObjectService
          .getInfo(id, layerName, additional)
          .then(function (response) {
            if (additional) {
              var arr = [
                "BUILDINGS_NUM",
                "PARCEL_ENTRANCES_NUM",
                "BUILDING_ENTRANCES_NUM",
              ];
              var actualChildElementsNum = {
                BUILDINGS_NUM: null,
                PARCEL_ENTRANCES_NUM: null,
                BUILDING_ENTRANCES_NUM: null,
              };
              arr.map(function (x) {
                if (response.data[x]) {
    ...
    this.getErrors = function () {
      ...
      function f() {
        ...
        simpleObjectService
          .getError(id, layerName, additional)
          .then(function (response) {
            if (additional) {
              var arr = [
                "BUILDINGS_NUM",
    ...
    this.pauseTask = function () {
      ...
      assignmentService.pauseTask(layerName, assignment_id).then(
        function () {
    ...
    this.split = function (callback) {
      ...
      features.map((x) =>
        FeatureArr.push({
          ...
          id:
            (x.get("featureType") || x.get("name")).substring(0, 10) ==
            "assignment"
              ? x.get("BLOCK_ID")
              : x.get("ID"),
        })
  ...
  initialControllerService.$inject = [
    "$window",
    "$rootScope",
    "StorageService",
    "MessagingService",
  ...
  angular
    ...
    .service("initialControllerService", initialControllerService);
})();
