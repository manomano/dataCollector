(function () {
  var parcelControllerService = function (
    $rootScope,
    ...
    function selectFilter(feature, layer) {
      return (
        (layer.get("name") == "parcels" &&
          feature.get("PARCEL_ID") == me.parcel_id) ||
        (!me.parcel_id && layer.get("name") == "existingParcels")
      );
    }
      ...
      if (act) {
        me.featureSelector = featureSelectorFactory(
          ...
          $rootScope.layerOrg.selectFilter({
            layers: ["existingObjects", "parcels", "all", "existingParcels"],
            layerName: "parcels",
            key: "ID",
            value: me.parcel_id,
          })
    ...
    this.RefreshOtherDependingLayers = function () {
      $rootScope.layerOrg.refreshLayers([
        "parcels",
        "buildings",
        "parcelEntrances",
        "buildingEntrances",
    ...
    this.back = function (geometry, rawFeature) {
      ...
      FormService.allowDataEntrance(
        me.title,
        { stepName: me.infoName, id: me.parcel_id },
        null,
        formCallBack,
    ...
    this.next = function () {
      ...
      simpleObjectService
        ...
        .then(
          function (response) {
            setIsactive(false);
            me.subscriber.trigger({
              who: "parcelController",
              parcel_id: me.parcel_id,
              buildings_count: me.buildings_count,
              parcelEntrances_count: me.parcelEntrances_count,
              parcelFeature: me.rawFeature,
              addedBuildingsCount: response.data,
            });
          },
          function () {}
        );
    ...
    this.getObjectCount = function () {
      return $rootScope.layerOrg.getObjectCount("parcels", {
        ID: me.parcel_id,
        PARCEL_ID: me.parcel_id,
        x: me.parcel_id,
      });
    ...
    this.draw = function () {
      if (me.getObjectCount() >= 1) {
        MessagingService.displayError(
          "თქვენ შეგიძლიათ მხოლოდ ერთი ნაკვეთი დახაზოთ!"
        );
        return;
      }
      setIsactive(false);
    ...
    this.copyGeometry = function () {
      ...
      var isCopied = $rootScope.layerOrg.checkIfCopied(
      ...
      if (assignmentFeature) {
        if (
          ...
          MessagingService.displayError(
            "მონიშნული ნაკვეთი ეკუთვნის სხვა დავალებას"
          );
          return;
        }
      }
    ...
    this.edit_general = function (event) {
      ...
      if (
        !feature ||
        !(
          feature.getProperties().name == "parcels" ||
          feature.getProperties().featureType == "parcel"
        )
      ) {
        MessagingService.displayError(
          "საჭიროა ნაკვეთის მონიშვნა ნაკვეთების ფენიდან"
        );
        return;
      }
  ...
  parcelControllerService.$inject = [
    "$rootScope",
    "DrawingControllerService",
    "$mdDialog",
    "ParcelService",
  ...
  angular
    ...
    .service("parcelControllerService", parcelControllerService);
})();
