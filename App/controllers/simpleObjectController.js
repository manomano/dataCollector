(function () {
  var simpleObjectController = function (
    $window,
    $rootScope,
    StorageService,
    ...
    function formCallBack(parameters) {
      ...
      simpleObjectService
        .metadata(me.id, parametersToPass, me.serviceName, me.editing)
        .then(
          function (response) {
            me.layerRefresh();
            me.stop();
            ...
            if (typeof imageUploader !== "undefined") {
              imageUploader.openImageUploadingPage(
                me.id,
                me.serviceName == "busStops" ? 2 : 3,
                StorageService.getObject("Authorization").accessToken
              );
            }
          },
          function (response) {
            ...
            MessagingService.displayError(
              "დაფიქსირდა პრობლემა, გთხოვთ სცადოთ მეტადატის შეყვანა კიდევ ერთხელ!"
            );
          }
        );
    }
    ...
    this.back = function (response) {
      ...
      if (me.serviceName == "pois") {
        FormService.allowDataEntranceHierarchial(
          me.title,
          me.infoName,
          null,
          formCallBack
    ...
    this.getObjectCount = function () {
      return $rootScope.layerOrg.getObjectCount(me.serviceName, {
        ID: me.id,
        x: me.id,
      });
    };
  ...
  angular
    ...
    .service("simpleObjectControllerService", simpleObjectController);
})();
