(function () {
    var controller = function ($scope, MessagingService, $mdDialog, assignmentService, user_id, assignment_ids, serviceName) {
        ...
        if (!assignment_ids.length) {
            assignmentService.getInfo($scope.serviceName, $scope.assignment_ids).then(function (response) {
        ...
        $scope.save = function () {
            ...
            assignmentService.assignUser($scope.serviceName, param).then(function () {
    ...
    controller.$inject = ["$scope", "MessagingService", "$mdDialog", "assignmentService", "user_id", "assignment_ids", "serviceName"];
    angular.module('datacollection.mypanel').controller("assignmentFormCtrl", controller);
})();
