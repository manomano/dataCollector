(function () {
  var buildingEntranceControllerService = function (
    $rootScope,
    ...
    function selectFilter(feature, layer) {
      return (
        layer.get("name") == "buildingEntrances" &&
        feature.get("BUILDING_ID") == me.building_id
      );
    }
      ...
      if (act) {
        ...
        me.featureSelector = featureSelectorFactory(
          ...
          $rootScope.layerOrg.selectFilter({
            layers: ["buildingEntrances", "all"],
            layerName: "buildingEntrances",
            key: "BUILDING_ID",
            value: me.building_id,
          })
        );
    ...
    this.updateToastTitle = function (additionalInfo) {
      $rootScope.listenerObject.currentTitle =
        "შენობის შესასვლელების ხაზვა  " + additionalInfo;
    };
    ...
    this.next = function () {
      simpleObjectService
        .getObjectCount("buildingEntrances", {
          x: me.building_id,
          ...
          function (response) {
            if (response.data < me.allowed_count) {
              MessagingService.displayError(
                "შესასვლელების რაოდენობა ნაკლებია მითითებულზე"
              );
              return;
            }

            simpleObjectService
              .metadataIsFilled("buildings", me.building_id)
    ...
    this.getObjectCount = function () {
      return $rootScope.layerOrg.getObjectCount("buildingEntrances", {
        x: me.building_id,
        BUILDING_ID: me.building_id,
      });
    };
    ...
    this.draw = function () {
      if (me.allowed_count <= me.getObjectCount()) {
        MessagingService.displayError(
          "შენობის შესასვლელების ოდენობა ემთხვევა მითითებულს, მეტის დამატების უფლება არ გაქვთ"
        );
        return;
      }
      setIsactive(false);
  ...
  buildingEntranceControllerService.$inject = [
    "$rootScope",
    "DrawingControllerService",
    "$mdDialog",
    "ParcelService",
