...
var WFSFeatureSelector = (function (_super) {
  ...
  function WFSFeatureSelector(map, layers, selectFilter) {
    var vectorLayers = layers.filter(
      (x) => x.type == "VECTOR" || x instanceof ol.layer.Vector
    );
  ...
  WFSFeatureSelector.prototype.initSelectInteraction = function (layers) {
    var layerForSnap = layers.filter(
      (x) => x.get("name") == "forDrawing" || x.get("name") == "selectionLayer"
    )[0];
  ...
  WFSFeatureSelector.prototype.setActiveByInteractionName = function (
    interactionKey,
    mode
  ) {
  ...
  WFSFeatureSelector.prototype.enableInteraction = function (
    interactionName,
    mode
  ) {
