...
var FeatureSelectorGeneral = (function () {
    ...
    return function (event) {
      ...
      self.getSelectedFeatures().forEach(function (feature) {
        if (
          ...
          event.selected[0].getProperties()["featureType"]
        ) {
          //self.clearSelection();
  ...
  FeatureSelectorGeneral.prototype.setActiveByInteractionName = function (
    interactionKey,
    mode
  ) {
  ...
  FeatureSelectorGeneral.prototype.enableInteraction = function (
    interactionName,
    mode
  ) {
