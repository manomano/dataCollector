...
var OnClickFeatureGetter = (function () {
    ...
    OnClickFeatureGetter.initInteraction = function () {
        ...
        this.map.on('singleclick', function (evt) {
            ...
            var visibleLayers = self.layers.filter(function (layer) { return layer.getVisible(); }).filter(function (layer) {
                return (layer.get('type')=='tile' || layer.type =='TILE' )
                    && layer.get('name')!=='OSM'
                    && layer.get('name').toLowerCase().indexOf("ortho")< 0
                    && layer.get('type')!=='ortho'
                    && (self.layerFilter?self.layerFilterIsActive && self.layerFilter.layers.indexOf(layer.get('name'))>=0:true)
            ...
            if(visibleLayers.find(x=>x.get('name')=='existingObjects')){
                ...
                var existingParcels = new AbstractLayerFactory(new GeoserverLayerFactory()).createLayer('existingParcels');
                var existingBuildings = new AbstractLayerFactory(new GeoserverLayerFactory()).createLayer('existingBuildings');
                ...
                var ind = visibleLayers.findIndex(x=>x.get('name')=='existingObjects');
            ...
            var x = function (layer) {
                ...
                var theLayer = new AbstractLayerFactory(new GeoserverLayerFactory()).createLayer(layer.get('name'));
                var url;


                if(theLayer){
                    url = theLayer.getSource().getGetFeatureInfoUrl(evt.coordinate, view.getResolution(), view.getProjection(), { 'INFO_FORMAT': 'text/javascript' });
                }
                ...
                return new Promise(function (resolve, reject) {
                    $.ajax({
                        ...
                        response.features.map(function (x) {
                            ...
                            x.properties.featureType = string.indexOf("_")>=0 ? string.replace("_"+string[string.indexOf("_")+1],string[string.indexOf("_")+1].toLocaleUpperCase()):string;
                            return x;
                        });
                        if(self.layerFilterIsActive && self.layerFilter){
                            ...
                            response.features = response.features.filter(function (x) {
                                return x.properties[self.layerFilter.key]==self.layerFilter.value || self.layerFilter.layers.filter(x=>x.indexOf("existing")>=0).indexOf(x.properties.featureType+'s')>=0
                            })
                        }
                        ...
                        $('body').append('<div id="popup"><div class="content">ინტერნეტთან წვდომა არ არის!</div></div>');

                        $('#popup').delay(2000).fadeOut(400, function () {
                            $('#popup').remove();
                        });
            ...
            function f(i){
                if(i >= visibleLayers.length){
                    ...
                    self.onSelect.trigger(OnClickFeatureGetter.clickCoordinates, selectedFeatures);
                    return;
                }
            ...
            x(visibleLayers[i]).then(function () { return f(i+1); })
