...
var layerOrganizerWFS = (function (_super) {
  ...
  layerOrganizerWFS.prototype.refreshLayers = function (
    layerNames,
    myCallBack
  ) {
  ...
  layerOrganizerWFS.prototype.getFeatureProperties = function (
    layerName,
    id,
    feature
  ) {
  ...
  layerOrganizerWFS.prototype.getFeaturesByCriteria = function (
    layerName,
    params
  ) {
    ...
    for (var i = 0; i < features.length; i++) {
      ...
      if (
        counter == parametersQ - 1 &&
        controllingArr.indexOf(features[i].N.ID) < 0
      ) {
  ...
  layerOrganizerWFS.prototype.selectFilter = function (params) {
    return function selectFilter(feature, layer) {
      return (
        (layer.get("name") == params.layerName &&
          feature.get(params.key) == params.value) ||
        layer.get("name").toLocaleLowerCase() == "existing" + params.layerName
      );
    };
  };
  ...
  layerOrganizerWFS.prototype.search = function (layerName, id) {
    ...
    if (layer) {
      var feature = layer
        ...
        .getFeatures()
        .find(function (x) {
          return layerName.toLowerCase().indexOf("assignments") >= 0
            ? x.get("BLOCK_ID") == id
            : x.get("ID") == id;
        });
...
var layerOrganizerWMS = (function (_super) {
  ...
  function layerOrganizerWMS(map, layers) {
    ...
    _this.urls = {
      ...
      buildingEntrances:
        utils.apiURL + admin + "pebe/buildingEntrances/x/metadata",

      existingParcels: utils.apiURL + admin + "pebe/existingParcels/x/metadata",
      existingBuildings:
        utils.apiURL + admin + "pebe/existingBuildings/x/metadata",
  ...
  layerOrganizerWMS.prototype.refreshLayers = function (layerName, myCallBack) {
    ...
    var layers = this.layers.filter(function (a) {
      return (
        layerName.indexOf(a.get("name")) >= 0 ||
        a.get("name").indexOf("all") >= 0
      );
    });
  ...
  layerOrganizerWMS.prototype.getFeatureProperties = function (
    layerName,
    id,
    callback
  ) {
  ...
  layerOrganizerWMS.prototype.getObjectCount = function (layerName, params) {
    ...
    var newURL =
      utils.apiURL + this.urls[layerName + "Num"].replace("x/", params.x + "/");
    var result = {};
    $.ajax({
      url: newURL,
      async: false,
  ...
  layerOrganizerWMS.prototype.metadataIsFilled = function (layerName, id) {
    ...
    $.ajax({
      url:
        utils.apiURL +
        this.urls[layerName + "MetadataIsFilled"].replace("x/", id + "/"),
      headers: {
        Authorization: userToken.getUserToken(),
        "Content-Type": "application/json",
      },
  ...
  layerOrganizerWMS.prototype.getFeatures = function (layerName, id) {
    ...
    var urls = {
      ...
      roadAssignmentsByGroupId:
        utils.apiURL + admin + "roadAssignments/groupId/x",
      roadAssignmentsByObjectId:
        utils.apiURL + admin + "roadAssignments/x/roadAssignments",
    };
  ...
  layerOrganizerWMS.prototype.search = function (layerName, id) {
    ...
    if (!feature.features) {
      ...
      feature.properties.featureType = layerName.substring(
        0,
        layerName.length - 1
      );
    } else {
  ...
  layerOrganizerWMS.prototype.getFeaturesByCriteria = function (layerName, o) {
    ...
    result.buildings.map(function (x) {
      features.push(
        new ol.format.GeoJSON({ featureProjection: "EPSG:3857" }).readFeatures(
          x.feature
        )[0]
      );
      return x;
...
var LayerUtils = (function () {
      ...
      loader: function (extent, resolution, projection) {
        ...
        xhr.onreadystatechange = function () {
          if (this.readyState == 4 && this.status == 200) {
            var preFeatures = new ol.format.GeoJSON({
              featureProjection: "EPSG:3857",
            ...
            preFeatures.map(function (el) {
              if (url.indexOf("pebe/") >= 0) {
                ...
                var ongoingAssignmentId = JSON.parse(
  ...
  LayerUtils.generateNGCACHETileLayer = function (
    layerName,
    urlTemplate,
    visibility
  ) {
    if (urlTemplate === void 0) {
      urlTemplate = NGCACHEServerInfo.NGCACHE_URL_TEMPLATE;
    }

    urlTemplate =
      urlTemplate + "&AutoKey=" + Math.random() + "&p=" + utils.placeId();
