var MapController = (function () {
    function MapController(layers, mapContainerID, defaultView) {
        ...
        OnClickFeatureGetter.addCallback(OnSelectFeatureChooser, OnSelectFeatureChooser.choose);
    ...
    MapController.prototype.initMap = function (target) {
        ...
        this.map.on('moveend', function () {
            ...
            var state = {
                ...
                time:new Date(),
                layers: self.layers.filter(x=>x.getVisible()).map(x=>x.get('name'))
            };
            ...
            if(typeof (sessionController)!=='undefined'){
                sessionController.setSession(JSON.stringify({"Authorization":JSON.parse(window[utils.getStorageType()].getItem("Authorization")), "mapState":state}));
            }
        });
    };
    ...
    MapController.prototype.select = function (selectType, value) {
        ...
        searcher.addOnCompleteCallback(this, function (responseBody) {
