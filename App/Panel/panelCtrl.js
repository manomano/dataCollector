(function () {
    ...
    var panelController = function($window, authService, $auth, $location, MessagingService, $scope, StorageService,  $rootScope, flowControllerService, initialControllerService, parcelControllerService, parcelEntranceControllerService, buildingControllerService, buildingEntranceControllerService, simpleObjectControllerService, DrawingControllerService){
        ...
        if(!auth){
            ...
            if(typeof (sessionController)!=='undefined'){
                sessionController.setSession(JSON.stringify({"Authorization":auth, "mapState":StorageService.getValue('mapState')}));
            }

        }

        $scope.roleDictionary = {"admin":"ROLE_ADMIN",  "pebe":"ROLE_PEBE_OPERATOR", "traffic":"ROLE_TRAFFIC_OPERATOR"};
        $scope.theRole = "";
        $scope.searchType = "";
        ...
        $scope.searchTypes = [
            ...
            {"title":"დავალება - გზები - groupId", "value":"roadAssignmentsByGroupId"},
            {"title":"დავალება - გზები - objectId", "value":"roadAssignmentsByObjectId"},
        ...
        if(auth){
            $scope.theRole = JSON.parse(StorageService.getValue('Authorization')).role;
        ...
        $rootScope.isWMS = JSON.parse(StorageService.hasProperty("isWMS")?StorageService.getValue('isWMS'):true);
        ...
        if($rootScope.isWMS) {
            $rootScope.layerOrg = new layerOrganizerWMS($rootScope.map, $rootScope.map.layers);
        }else{
        ...
        if(StorageService.getObject('Authorization').userId ==6){
            ...
            for(var i=0; i<$rootScope.map.layers.length; i++){
                 if(['pois','all', 'ORTHO_2014_DASAVLETI', 'existingParcels', 'existingObjects', 'assignments'].indexOf($rootScope.map.layers[i].get('name'))>=0){
        ...
        if(StorageService.getObject('mapState') && StorageService.getObject('mapState').layers){
        ...
        $scope.orthoPairValues = {0:{value:"", lastValue:""},1:{value:"", lastValue:""}};


        $scope.orthoPairs = [
            [{key:'ORTHO_2014_DASAVLETI', title:'ORTHO 2014 WMTS'},   {key:"ORTHO_2014_DASAVLETI_WMS",title:"ORTHO 2014 WMS"}, {key:"none|0", title:"arcerti"}],
            [{key:"ORTHO_2016_17_NORV", title:"ORTHO 2016 WMS"},{key:"ORTHO_2016_17_NORV_WMTS",title:"ORTHO 2016 WMTS"},{key:"none|1", title:"arcerti"}],
            [{key:"ORTHO_2015_SAMEGRELO_WMS", title:"ORTHO 2015 samegrelo"},{key:"ORTHO_2015_SAMEGRELO_WMTS",title:"ORTHO 2015 samegrelo WMTS"},{key:"none|1", title:"arcerti"}]
        ];
        ...
        $rootScope.map.layers.map(function (el) {
            if(el.get('name')){
                ...
                if(label){
                    $scope.layerChecks.push({name:label,model:el.get('visible'), value:name});
                }
                if($scope.orthoOpposites.hasOwnProperty(name)){
                    if(el.get('visible')){
                        $scope.orthoPairValues[(name.indexOf("NORV")>=0?1:0)].value = name;
                    }
                }
            }
        ...
        $scope.selectOrtho = function(selected, it){
            ...
            if(selected.value.substring(0,4)=="none"){
                ...
                $scope.changeLayerVisibilityByValue ($scope.orthoOpposites[selected.value],false);
            }
        }
        ...
        $scope.search = function () {
            ...
            if($scope.searchType && $scope.searchable_id){
                var feature = $rootScope.layerOrg.search($scope.searchType, $scope.searchable_id);
                ...
                if(feature){
                    ...
                    var extent = !feature.length?feature.getGeometry().getExtent(): source.getExtent();
                    $rootScope.map.map.getView().fit(extent);


                    if(!feature.length) {
                        $scope.initialControllerService.featureSelector.selectFeature(feature);
                    }else{
                        $scope.initialControllerService.featureSelector.selectFeatures(feature);
                    }
                    return;
                }
        ...
        $scope.buildingEntranceControllerService = buildingEntranceControllerService;
    ...
    panelController.$inject = ["$window","authService","$auth","$location","MessagingService", "$scope", "StorageService", "$rootScope", "flowControllerService", "initialControllerService", "parcelControllerService", "parcelEntranceControllerService", "buildingControllerService", "buildingEntranceControllerService", "simpleObjectControllerService","DrawingControllerService"];

    angular.module("datacollection.mypanel").controller("MainPanelCtrl", panelController);
